 ...
 */
 ...
 任意初始值：它的“零点”不是固定的 0，而是可以任意定义的一个初始值。这意味着计数器可以从任意设定的值开始，而不是从传统意义上的 0 开始。
 回绕行为：当该整数的值达到最大值 2^32 - 1 后，不会发生溢出（即转为负数或其他错误），而是自动回绕到最初设定的零点，然后继续递增。这种行为类似于循环计数器，保证在达到上限后能够顺利返回起始状态。
 */
...
public:
   ...
   */
   //unwrap 的主要任务就是找出最接近 checkpoint 的那一个绝对序列号，使得当使用相应的 wrap 方法转换后，能够得到当前的 Wrap32 值。
